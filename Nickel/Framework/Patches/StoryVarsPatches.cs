	...
	internal static void Apply(Harmony harmony)
	{
		harmony.Patch(
			original: AccessTools.DeclaredMethod(typeof(StoryVars), nameof(StoryVars.GetUnlockedChars)) ??
					  throw new InvalidOperationException(
						  "Could not patch game methods: missing method `StoryVars.GetUnlockedChars`"
					  ),
			postfix: new HarmonyMethod(
				AccessTools.DeclaredMethod(typeof(StoryVarsPatches), nameof(GetUnlockedChars_Postfix))
			)
		);
		...
		harmony.Patch(
			original: AccessTools.DeclaredMethod(typeof(StoryVars), nameof(StoryVars.GetUnlockedShips)) ??
					  throw new InvalidOperationException(
						  "Could not patch game methods: missing method `StoryVars.GetUnlockedShips`"
					  ),
			postfix: new HarmonyMethod(
				AccessTools.DeclaredMethod(typeof(StoryVarsPatches), nameof(GetUnlockedShips_Postfix))
			)
		);
	}
